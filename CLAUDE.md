...
**変更したらすぐコミットする。** 1つのタスク（Issue）を1コミットにまとめない。
...
M2 以降のタスクは GitHub Issue にも起票してある（Milestone で絞る）。
**Issue を1件実装するときは `/issue <番号>` を使う**（`.claude/commands/issue.md`）。
ブランチを切るところから PR を出すところまで、以下の規約に沿って進む。

刻み方の目安（1つのタスクの中でも、これらは別々のコミットにする）:

| 刻む単位 | 例 |
|---|---|
| スキーマ | マイグレーション + `docs/schema.sql` |
| データアクセス層 | `Store` のメソッドとそのテスト |
| API1経路 | ハンドラ + 経路の登録 + そのテスト |
| 型・APIクライアント | `web/src/api/` の追加 |
| 画面1つ | `web/src/screens/` の追加 + そのテスト |
| 既存テストの更新 | 挙動を変えたことで直したもの |

**コミットは細かく、PRは機能単位で束ねる。** レビューする単位（PR）と、
後から履歴を辿る単位（コミット）は別物でよい。
...
- **そのコミット単体で意味が完結している。** 「書きかけの関数」をコミットしない。
  **ただし「機能が完成していること」までは求めない。** 型だけ、APIクライアントだけ、
  画面だけ、のコミットは意味が完結している
- **押せないボタンを含めない。** UIを追加するコミットは、そのUIが機能する状態にする。
  経路に繋いでいない画面はボタンとして存在しないので、これに反しない
- 検査（`fmt` / `test`）は**コミットの前に通す**。通らないものをコミットしない
...
- **タスク（Issue）ごとに、そこから作業ブランチを切る**（`m2-2-loan-api`）。
  1タスク1コミットを積み、マイルストーンブランチへ PR を出してマージする。
  **区切りはハイフン。`m2/2-loan-api` は作れない**（`m2` ブランチが既にあると、
  Git が `refs/heads/m2` をディレクトリにできず ref が衝突する）
- マイルストーンの受け入れ条件を**全て満たしてから** `main` へマージし、タグを打つ（`v0.1.0-m1`）
- **`main` は常にビルドが通り、受け入れ条件を満たした状態を保つ**

**マイルストーンブランチへのマージでは Issue が自動で閉じない。**
GitHub の自動クローズは既定ブランチ（`main`）へのマージにしか効かない。
PR 本文に `Closes #N` を書いたうえで、マージ後に `gh issue close N` で閉じる。
閉じ忘れると、実装済みの Issue が残り続けて次に何をやるかが読めなくなる。
...
- [x] ラベルPDF（A4 24面 / 70×33.9mm、QR + コード + 品名）
      （`label.PDF`。面付けは用紙が決めるため定数に閉じる。左右の内側余白5mmは
      プリンタの印字不可領域を避けるため。品名は2行で折り返し、溢れたら「…」）
- [x] 印刷範囲の指定（コード範囲・分類）
      （`GET /api/labels.pdf?from=&to=&category=`。admin のみ。**コード範囲は数値で比較する。**
      文字列だと "10000" < "9999" になり、4桁を超えた備品が静かに漏れる。
...
- [x] APIクライアント + レスポンス型定義 + 認証状態の管理
      （`web/src/api/` と `web/src/auth/`。**経路ごとの関数は画面を作る時に足す。**
      使う画面が無いうちに全経路を書いても、動かしたことがないコードが残る。
      401 は `onSessionExpired` で1箇所に集める。画面ごとに拾わせると必ず
      拾い忘れた画面ができる。`next` の解釈はサーバに任せ `redirect_to` へ進む）
- [x] `/login` と `/password`
      （`RequireAuth` でログインの要否を経路ごとに決める。**画面を足す時は必ず判断する。**
      包み忘れた画面はAPIが401を返すまで中身を出し続ける。
      **`/api/password` の401は「現在のパスワードが違う」でセッション切れではない。**
      同じ扱いにすると、打ち間違えた人がその場でログアウトさせられる）
- [x] `/items`（一覧・検索。分類・保管場所・状態でフィルタ）
      （絞り込みはURLのクエリに持つ。画面の中だけに持つと戻る操作で条件が消える。
      **良好・在庫の目印は出さない。** 全行に付くと注意すべき行が埋もれる。
      **詳細へのリンクはまだ置かない**（`/i/{code}` が無いため）。次のタスクで足す）
- [x] `/i/{code}`（**M1では表示のみ。貸出ボタンを置かない**）
      （**この経路は変えられない。** ラベルは貼り替えられないため、URLを変えると
      印刷済みのQRが全て読めなくなる。廃棄済みもここでは引ける。
      **ログイン画面での復帰は `navigate()` ではなく `<Navigate>` で行う。**
      命令的に呼ぶと「ログイン済みならトップへ」と競り、QRから来た人が
      トップへ放り出される）
- [x] `/admin/items`（マスタ管理）
      （`RequireAdmin` は中で `RequireAuth` を通す。並べて書くと片方だけ書いた経路ができる。
      **削除の導線を置かない。** 廃棄は状態で、行を消すと貸出履歴の参照先が消える。
      **更新は全項目を送る。** 廃棄も状態の1つとしてここで指定し、
      `POST /api/items/{code}/discard` は使わない。状態だけ別経路にすると、
      廃棄済みを編集した時に状態を送り戻して復活させる経路ができる）
- [x] `/admin/items/new`（登録フォーム、写真添付）
      （**備品コードを入力させない。採番された値は登録後に必ず表示する**（ラベルに要る）。
      写真は備品を登録した後にしか送れない。**写真だけ失敗しても登録はやり直させない。**
      送り直すと同じ備品が2件でき、採番が1つ無駄になる（番号は再利用しない）。
      入力欄は `ui/ItemFields` に集約し、登録と編集で同じものを使う）
- [x] `/admin/items/import`（プレビュー → 確定）
      （**確定の前に必ずプレビューを挟む。** 同じファイルを2度送る（サーバは解析結果を持たない）。
      **採番の予定は「予定」と明記し、除外を選んだら出さない。** 決まった値に見せると
      その範囲でラベルを刷る人が出る。誤りは行番号付きで全件出し、1件でもあれば取り込ませない。
      記入例の消し忘れは「取り込まない」の印で除く（CSVを直させない））
- [x] `/admin/labels`（範囲指定 → PDF出力）
      （**PDFはリンクで開く。** fetch して組み立て直さない。サーバは `inline` で返しており、
      ブラウザのPDFビューアで確認してから印刷できることが狙い。
      **対象0件ならリンクを出さない。** 白紙のシートを刷るとラベルシールが1枚無駄になる。
      **範囲は数値で比較する**（画面側も）。文字列だと "10000" < "9999" になる）
- [x] `/admin/users`（ユーザー管理）
      （**初期パスワードは控えたと明示するまで画面から消さない。** 応答でしか手に入らない。
      **削除の導線を置かない**（卒業者は無効化）。最後の admin の無効化は 409 をそのまま出す。
...
- [x] `docker compose up` だけで起動し、**ブラウザからログインまで通ることを確認**
      （`.env` を用意して `docker compose up -d` だけで healthy になる。未ログインで
      `/i/0002` を開くと `/login?next=%2Fi%2F0002` へ復帰付きで飛ぶことをブラウザで確認済み。
      ログイン〜画面遷移はAPI経路として全て検証済み（下記の受け入れ確認））
- [x] コンテナ内での `-create-admin` 実行手順を確立（`docker compose exec`）
      （`make create-admin LOGIN_ID= NAME=` / `.\make.ps1 create-admin -LoginId -Name`。
      **scratch にシェルが無いため `exec app /server ...` の形でしか呼べない。**
      パスワードは引数で渡さず、生成して一度だけ表示する）
- [x] バックアップ/リストアの実演（`-backup` を取り、**そこから復元できることまで確認する**）
      （`-restore` をバイナリ側に追加した。ホストから `cp` して `chown` する手順は
      シェルが無いぶん成立しない。**`down -v` でボリュームごと消してから復元し、
      バックアップ時点のパスワードでログインでき、備品6件が戻ることまで確認済み。**
      `make backup` の PowerShell 版が `$?` を見ていて取り出しが飛んでいたのを
      `$LASTEXITCODE` で判定するよう修正した（docker は stderr に書くため））
- [x] README（セットアップ・環境変数・**バックアップ手順**・admin追加手順・引き継ぎ事項）
      （「開発する」と「運用する」を分けた。運用側は Docker だけで完結する手順にする。
      **`HOST_URL` は後から変えられない**ことと、`COOKIE_SECURE` を落とさないと
      HTTP運用でログインループになることを明記した）
- [ ] 受け入れ条件の実機確認（**ラベルを実際に印刷し、実スマホの標準カメラで読む**）
      （**物理確認のみ残り。** それ以外の受け入れ条件12項目は本番Composeの実物に対して
      通しで検証済み（`docs/m1-implementation-spec.md §9`）。ラベルPDFは
      QR + コード + 日本語品名がA4 24面に出ることをブラウザで目視確認済み）

### M2: 貸出・返却

`docs/m2-implementation-spec.md` が仕様。**M1の受け入れ条件を全て満たしてから着手する。**

- [x] SMTP 送信の基盤（環境変数で接続先。**未設定なら送信をスキップして動く**）
      （`internal/notify`。`SMTP_HOST` が空なら送らずに nil を返す。**呼び出し側に
      分岐を書かせない。** 各所に「設定されていれば送る」と書くと書き忘れた経路ができる。
      **途中まで設定された状態では起動しない**（`SMTP_FROM` 欠け・認証情報の片方だけ）。
      送れているつもりで送れていない状態が一番気付きにくい。件名は改行を落としてから
      符号化する。**ASCII だけの件名は符号化されず素通りするため、ヘッダを注入できる**）
- [x] 借用API（返却予定日は借用日+14日をデフォルト。**二重貸出を409で返す。500にしない**）
      （`0002_loans.sql` と `internal/loan`。二重貸出は部分ユニークインデックスで拒否し、
      違反を 409 に変換する。SELECT してから INSERT する形は競合に負ける。
      返却予定日は **JST の日付**で計算する。UTC だと JST 00:00〜09:00 の分が1日ずれる）
- [x] 自由利用品への借用APIを400で拒否（**UIで隠すだけにしない**）
      （借用APIと同じコミットに含めた。これを欠いたコミットは
      「押せば自由利用品も借りられてしまうAPI」として履歴に残る）
- [x] 所在不明品の借用時に、在庫へ自動復帰（**追加操作を求めない**）
      （借用と同一トランザクション。分けると、借用だけ通って復帰が漏れた行が残る。
      **「報告を発見済みへ」は M3。** `missing_reports` は M2 に存在しないため、
      この自動復帰に M3 で足す（m2-spec §3, §13））
- [x] 返却API（**借用者以外も実行でき、`returned_by` に記録する**）
      （`POST /api/items/{code}/return`。本文を読まない。**貸出中でなければ 409。**
      200 で黙って成功にすると「返した気になっているが記録が無い」状態を作る。
      廃棄済みでも返せる。持ち出したまま廃棄になった備品を戻せないと貸出中の行が残る）
- [x] 代理登録（`registered_by` を記録）+ 本人へのメール通知 + 訂正導線
      （記録は借用APIに含めた。ここでは通知と取り消しAPI（`POST /api/loans/{id}/cancel`）。
      **取り消しは返却ではない。** `cancelled_at` を立てて行は残す。返却として記録すると、
      借りていない人の返却履歴が残り、破損の追跡で誤った経路をたどる。
      **送信の失敗で借用を巻き戻さない。** 画面の導線は `/loans/mine`（M2-9）で足す）
- [x] 事後登録（`borrowed_at` を過去日時に変更できる）
      （APIは借用APIに含めて実装した。**画面の導線は `/loans`・`/loans/mine` で足す**——
      トップから1タップで届く位置に置くこと。「もう持ち出しちゃったから今さら」を潰すのが目的）
- [x] 破損報告API（報告と同時に `condition = '要修理'`。承認を待たない）
      （`internal/report`。報告は全員、追認（`GET /api/damages` /
      `POST /api/damages/{id}/status`）は admin。**既に廃棄なら要修理に戻さない。**
      廃棄は終端状態で、報告で復活すると廃棄したはずのものが一覧に現れる。
      **写真は未実装**（#13 の未確定事項。決まるまで説明文のみ））
- [x] 貸出中一覧API / 自分の貸出API
      （`GET /api/loans`（全員）と `GET /api/loans/mine`。**取り消し済みはどちらにも出さない。**
      一覧に出すと誤登録が「起きた事実」として全員に見え続ける。
      備品詳細の `loan` は **main で繋ぐ**（item と loan は互いを参照しない。
      参照すると応答に載せる備品の形で循環する））
- [x] `/i/{code}` の状態分岐（貸出可能・貸出中他人・貸出中自分・自由利用品・廃棄・所在不明）
      （借用は `/i/{code}/borrow` で確定する。**廃棄済みでも自分が借りていれば返せる**
      ——持ち出したまま廃棄になった分を戻せないと貸出中の行が永久に残る。
      破損報告もこの画面に常設した）
- [x] 借用画面に**現在の状態と備考を表示**（責任の誤帰属を防ぐ）
      （代理登録の選択欄は「変更する」の内側に置く。既定は自分のまま。
      選択肢は `GET /api/members`（member も引ける。**IDと名前だけ**）。
      `GET /api/users` は admin 限定のまま変えない——1つのハンドラが権限で
      違う形を返すと、項目を足した時に member 向けへ漏れる）
- [x] `/loans`（全員に公開）と `/loans/mine`
      （絞り込みはURLのクエリに持つ。**期限内であることの印は出さない**——
      全行に付く印は注意すべき行を埋もれさせる。
      **返却と取り消しを同じボタンにまとめない。** 「返した」と「そもそも借りていない」は
      別の事実。返却は確認を挟まず、取り消しだけ挟む。
      トップに事後登録の入口を置いた）
- [x] `/admin/damages`（破損報告の追認）
      （**承認ではなく追認。** 既定は未確認だけを出す（全件を既定にすると
      処理済みが積み上がって埋もれる）。**状態と備品の連動を押す前に書く**——
      書かないと直したつもりで廃棄にする人が出る。削除の導線は置かない）
